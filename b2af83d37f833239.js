(globalThis.TURBOPACK || (globalThis.TURBOPACK = [])).push(["object" == typeof document ? document.currentScript : void 0, 48949, e => {
    "use strict";
    var t = e.i(43476)
      , a = e.i(71645)
      , s = e.i(55487);
    e.i(47167);
    var r = e.i(46932)
      , l = e.i(48148)
      , o = e.i(5427)
      , n = e.i(16715)
      , i = e.i(95187)
      , c = (0,
    i.createServerReference)("602c60c9e18b0a3dbbcd0ee55f01c385f88d0638e0", i.callServer, void 0, i.findSourceMapURL, "submitMobileLogin")
      , u = e.i(97476)
      , d = e.i(76275)
      , p = e.i(32252);
    function m({text: e, width: s=180, height: r=50}) {
        let l = (0,
        a.useRef)(null)
          , o = (0,
        a.useCallback)( () => {
            let t = l.current;
            if (!t)
                return;
            let a = t.getContext("2d");
            if (!a)
                return;
            a.fillStyle = "#f0f0f0",
            a.fillRect(0, 0, s, r);
            for (let e = 0; e < 100; e++)
                a.fillStyle = `rgba(${100 * Math.random() + 100}, ${100 * Math.random() + 100}, ${100 * Math.random() + 100}, 0.5)`,
                a.beginPath(),
                a.arc(Math.random() * s, Math.random() * r, 2 * Math.random(), 0, 2 * Math.PI),
                a.fill();
            for (let e = 0; e < 5; e++)
                a.strokeStyle = `rgba(${150 * Math.random() + 50}, ${150 * Math.random() + 50}, ${150 * Math.random() + 50}, 0.4)`,
                a.lineWidth = 2 * Math.random() + 1,
                a.beginPath(),
                a.moveTo(Math.random() * s, Math.random() * r),
                a.bezierCurveTo(Math.random() * s, Math.random() * r, Math.random() * s, Math.random() * r, Math.random() * s, Math.random() * r),
                a.stroke();
            let o = e.split("")
              , n = (s - 20) / o.length;
            o.forEach( (e, t) => {
                a.save();
                let s = 10 + t * n + 4 * Math.random() - 2
                  , l = r / 2 + 10 * Math.random() - 5;
                a.translate(s, l),
                a.rotate((Math.random() - .5) * .4);
                let o = Math.floor(8 * Math.random()) + 20
                  , i = ["Arial", "Georgia", "Verdana", "Times New Roman", "Courier"]
                  , c = i[Math.floor(Math.random() * i.length)];
                a.font = `bold ${o}px ${c}`;
                let u = 360 * Math.random();
                a.fillStyle = `hsl(${u}, 70%, 30%)`,
                a.shadowColor = "rgba(0, 0, 0, 0.3)",
                a.shadowBlur = 2,
                a.shadowOffsetX = 1,
                a.shadowOffsetY = 1,
                a.fillText(e, 0, 0),
                a.restore()
            }
            );
            for (let e = 0; e < 3; e++)
                a.strokeStyle = `rgba(${100 * Math.random() + 100}, ${100 * Math.random() + 100}, ${100 * Math.random() + 100}, 0.3)`,
                a.lineWidth = 1,
                a.beginPath(),
                a.moveTo(0, Math.random() * r),
                a.lineTo(s, Math.random() * r),
                a.stroke()
        }
        , [e, s, r]);
        return (0,
        a.useEffect)( () => {
            o()
        }
        , [o]),
        (0,
        t.jsx)("canvas", {
            ref: l,
            width: s,
            height: r,
            className: "rounded-md border select-none",
            style: {
                imageRendering: "pixelated"
            }
        })
    }
    let h = {
        hidden: {
            opacity: 0,
            y: 40
        },
        visible: {
            opacity: 1,
            y: 0,
            transition: {
                duration: .6,
                ease: "easeOut"
            }
        }
    };
    function g() {
        let e, t, a, s = ["+", "-", "×"], r = s[Math.floor(Math.random() * s.length)];
        switch (r) {
        case "+":
            a = (e = Math.floor(50 * Math.random()) + 1) + (t = Math.floor(50 * Math.random()) + 1);
            break;
        case "-":
            t = Math.floor(Math.random() * (e = Math.floor(50 * Math.random()) + 10)),
            a = e - t;
            break;
        case "×":
            a = (e = Math.floor(12 * Math.random()) + 1) * (t = Math.floor(12 * Math.random()) + 1);
            break;
        default:
            e = 1,
            t = 1,
            a = 2
        }
        return {
            num1: e,
            num2: t,
            operator: r,
            answer: a,
            display: `${e} ${r} ${t} = ?`
        }
    }
    function x() {
        let e = (0,
        s.useDispatch)()
          , i = (0,
        s.useSelector)(e => e.ip.clientIp)
          , x = (0,
        l.useTranslations)("root")
          , [b,f] = (0,
        a.useState)("")
          , [y,v] = (0,
        a.useState)("")
          , [w,j] = (0,
        a.useState)(!1)
          , [S,N] = (0,
        a.useState)(null)
          , [C,k] = (0,
        a.useState)("")
          , [P,M] = (0,
        a.useState)( () => g())
          , [D,I] = (0,
        a.useState)(!1)
          , T = y.trim().length >= 11
          , _ = (0,
        a.useCallback)( () => {
            M(g()),
            k(""),
            I(!1)
        }
        , [])
          , O = async t => {
            if (t.preventDefault(),
            Number.parseInt(C, 10) !== P.answer) {
                I(!0),
                _();
                return
            }
            try {
                j(!0),
                N(null);
                let t = await c({
                    mobile_no: y,
                    captcha_token: b,
                    answer: P.answer,
                    user_input: C
                }, i || void 0);
                t?.status_code === 200 ? (e((0,
                d.setStep)(t.data?.registered ? 3 : 2)),
                e((0,
                u.setPhoneNumber)(y))) : (N({
                    message: t.message,
                    type: "error"
                }),
                _())
            } catch (e) {
                N({
                    message: e instanceof Error ? e.message : "An unexpected error occurred.",
                    type: "error"
                }),
                _()
            } finally {
                j(!1)
            }
        }
          , R = "" !== C.trim() && Number.parseInt(C, 10) === P.answer;
        return (0,
        t.jsxs)(r.motion.div, {
            className: "bg-white w-full p-4 rounded-lg ",
            variants: h,
            initial: "hidden",
            animate: "visible",
            children: [S && (0,
            t.jsx)(p.default, {
                message: S.message,
                type: S.type,
                onClose: () => N(null)
            }), (0,
            t.jsxs)("p", {
                className: "text-xs mb-2",
                children: [x("lblInputLoginMobile"), " ", (0,
                t.jsx)("span", {
                    className: "text-red-600 text-[10px]",
                    children: x("lblInputLoginMobileMsg")
                })]
            }), (0,
            t.jsxs)("form", {
                onSubmit: O,
                children: [(0,
                t.jsx)("input", {
                    type: "hidden",
                    name: "lang"
                }), (0,
                t.jsx)("input", {
                    type: "tel",
                    name: "phone",
                    className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-sm focus:ring-0 focus:border-slate-400 duration-300 border-[1.5px] focus:outline-0 block w-full p-2.5 mb-3",
                    placeholder: x("lblInputLoginMobile"),
                    autoComplete: "off",
                    inputMode: "numeric",
                    maxLength: 11,
                    pattern: "[0-9]*",
                    value: y,
                    onChange: e => {
                        v(e.target.value.replace(/\D/g, "").slice(0, 11))
                    }
                    ,
                    onCopy: e => e.preventDefault(),
                    onPaste: e => e.preventDefault(),
                    onCut: e => e.preventDefault(),
                    required: !0
                }), (0,
                t.jsx)("div", {
                    className: "mb-2",
                    children: (0,
                    t.jsx)(o.default, {
                        sitekey: "0x4AAAAAABpNUpzYeppBoYpe",
                        onVerify: e => f(e)
                    })
                }), (0,
                t.jsxs)("div", {
                    className: "mb-3",
                    children: [(0,
                    t.jsx)("p", {
                        className: "text-xs mb-2 text-gray-600",
                        children: "Solve the math problem below:"
                    }), (0,
                    t.jsxs)("div", {
                        className: "flex items-center gap-2 mb-2",
                        children: [(0,
                        t.jsx)(m, {
                            text: P.display,
                            width: 160,
                            height: 45
                        }), (0,
                        t.jsx)("button", {
                            type: "button",
                            onClick: _,
                            className: "p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-md transition-colors cursor-pointer",
                            "aria-label": "Refresh captcha",
                            children: (0,
                            t.jsx)(n.RefreshCw, {
                                className: "w-4 h-4"
                            })
                        })]
                    }), (0,
                    t.jsx)("input", {
                        type: "text",
                        inputMode: "numeric",
                        pattern: "[0-9]*",
                        value: C,
                        onChange: e => {
                            k(e.target.value.replace(/[^0-9-]/g, "")),
                            I(!1)
                        }
                        ,
                        placeholder: "Enter answer",
                        className: `bg-white h-8 text-gray-900 font-light text-xs rounded-sm focus:ring-0 duration-300 border-[1.5px] focus:outline-0 block w-full p-2.5 ${D ? "border-red-500 focus:border-red-500" : "border-gray-300 focus:border-slate-400"}`,
                        required: !0,
                        style: {
                            width: "25.5%"
                        },
                        onCopy: e => e.preventDefault(),
                        onPaste: e => e.preventDefault(),
                        onCut: e => e.preventDefault()
                    }), D && (0,
                    t.jsx)("p", {
                        className: "text-red-500 text-[10px] mt-1",
                        children: "Incorrect answer. Please try again."
                    })]
                }), (0,
                t.jsx)(r.motion.button, {
				type: "submit",
				disabled: w, 
				whileTap: {
				scale: 0.95
				},
				whileHover: {
				scale: !w ? 1.03 : 1
				},
				className: `text-white mb-2 h-8 rounded-lg  text-xs w-full duration-300 sm:w-auto px-2 py-2 text-center transition 
				${w ? "bg-gray-400 cursor-not-allowed" : "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838] cursor-pointer"}`,
				children: w ? x("checking") + "..." : x("lblProceed")
				})
				})]
			})]
        })
    }
    var b = (0,
    i.createServerReference)("607e554c5e3b0872eddde365f294b534455946cb46", i.callServer, void 0, i.findSourceMapURL, "registrationOtp")
      , f = (0,
    i.createServerReference)("605d7bfc835abf06e52e5d6850aa6bd09ed3e671e2", i.callServer, void 0, i.findSourceMapURL, "reSendOtp")
      , y = e.i(88653)
      , v = e.i(63762)
      , w = e.i(59459);
    let j = ({message: e, type: s="info", onClose: l}) => {
        let[o,n] = (0,
        a.useState)(!0);
        if (!o)
            return null;
        let i = (0,
        w.isHTML)(e) ? "Too many attempts. Please try again later." : e;
        return (0,
        t.jsx)(y.AnimatePresence, {
            children: o && (0,
            t.jsx)(r.motion.div, {
                initial: {
                    scale: .8,
                    opacity: 0
                },
                animate: {
                    scale: 1,
                    opacity: 1
                },
                exit: {
                    scale: .8,
                    opacity: 0
                },
                transition: {
                    type: "spring",
                    stiffness: 150,
                    damping: 25
                },
                className: "fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm",
                children: (0,
                t.jsxs)("div", {
                    className: `flex justify-center flex-col h-1/2 rounded-xl shadow-2xl p-6 max-w-md w-full mx-4 text-center ${{
                        success: "bg-green-500 text-white",
                        error: "bg-red-500 text-white",
                        info: "bg-blue-500 text-white",
                        warning: "bg-yellow-400 text-black"
                    }[s]}`,
                    children: [(0,
                    t.jsxs)("div", {
                        className: "flex flex-col justify-center items-center mb-4",
                        children: ["error" === s && (0,
                        t.jsx)(v.CloseIcon, {}), "success" === s && (0,
                        t.jsx)(v.SuccessIcon, {}), (0,
                        t.jsxs)("p", {
                            className: "text-white font-bold text-2xl py-4",
                            children: ["error" === s && "Error!", "success" === s && "Success!"]
                        })]
                    }), (0,
                    t.jsx)("span", {
                        className: "block mb-4",
                        children: i
                    }), (0,
                    t.jsx)("button", {
                        onClick: () => {
                            n(!1),
                            "Session expired. Please log in again." === e && (localStorage.clear(),
                            window.location.href = "/"),
                            "Registration successful" === e && (localStorage.clear(),
                            window.location.href = "/"),
                            "Password changed successfully" === e && (localStorage.clear(),
                            window.location.href = "/"),
                            l && l()
                        }
                        ,
                        className: "mt-2 w-24 py-2 rounded bg-gray-500 text-white hover:bg-gray-600 mx-auto",
                        children: "OK"
                    })]
                })
            })
        })
    }
      , S = () => {
        let e = (0,
        s.useDispatch)()
          , o = (0,
        l.useTranslations)("root")
          , n = (0,
        s.useSelector)(e => e.authUser.user)
          , i = (0,
        s.useSelector)(e => e.ip.clientIp)
          , [c,u] = (0,
        a.useState)(!1)
          , [p,m] = (0,
        a.useState)("")
          , [h,g] = (0,
        a.useState)("")
          , x = "" !== p.trim() && p.length >= 6
          , [y,v] = (0,
        a.useState)(null)
          , [w,S] = (0,
        a.useState)(30)
          , [N,C] = (0,
        a.useState)(!0)
          , [k,P] = (0,
        a.useState)(!1);
        (0,
        a.useEffect)( () => {
            n?.user_phone && g(n.user_phone)
        }
        , [n]);
        let M = async () => {
            u(!0);
            try {
                let e = await f({
                    resend: "1"
                }, i || void 0);
                200 === e.status_code ? (S(30),
                C(!0),
                P(!1)) : (S(30),
                C(!0),
                P(!1),
                v({
                    message: e?.message,
                    type: "error"
                }))
            } catch (s) {
                let t = "object" == typeof s && s?.message ? s.message : "Failed. Please try again later."
                  , a = "object" == typeof s && "number" == typeof s.status ? s.status : 422;
                (401 === a || 419 === a) && (v({
                    message: t,
                    type: "error"
                }),
                setTimeout( () => {
                    e((0,
                    d.setStep)(1))
                }
                , 5e3)),
                v({
                    message: t,
                    type: "error"
                })
            } finally {
                u(!1)
            }
        }
          , D = async () => {
            u(!0);
            try {
                let e = await b({
                    otp: p
                }, i || void 0);
                200 === e.status_code ? v({
                    message: "Registration successful",
                    type: "success"
                }) : v({
                    message: e?.message,
                    type: "error"
                })
            } catch (a) {
                let e = "object" == typeof a && a?.message ? a.message : "Failed. Please try again later."
                  , t = "object" == typeof a && "number" == typeof a.status ? a.status : 422;
                (401 === t || 419 === t) && (v({
                    message: e,
                    type: "error"
                }),
                setTimeout( () => {
                    (0,
                    d.setStep)(1)
                }
                , 5e3)),
                v({
                    message: e,
                    type: "error"
                })
            } finally {
                u(!1)
            }
        }
        ;
        return (0,
        a.useEffect)( () => {
            let e;
            return N && w > 0 && (e = setInterval( () => {
                S(e => e - 1)
            }
            , 1e3)),
            0 === w && N && (clearInterval(e),
            C(!1),
            P(!0)),
            () => clearInterval(e)
        }
        , [w, N]),
        (0,
        t.jsxs)(r.motion.div, {
            className: "bg-white w-full p-4",
            variants: {
                hidden: {
                    opacity: 0,
                    y: 40
                },
                visible: {
                    opacity: 1,
                    y: 0,
                    transition: {
                        duration: .6,
                        ease: "easeOut"
                    }
                }
            },
            initial: "hidden",
            animate: "visible",
            children: [y && (0,
            t.jsx)(j, {
                message: y.message,
                type: y.type,
                onClose: () => v(null)
            }), (0,
            t.jsxs)("p", {
                className: "text-[10px]",
                children: [o("lblAuthOtp1"), " (", h, ")"]
            }), (0,
            t.jsx)("p", {
                className: "text-[10px]",
                children: o("lblAuthOtp2")
            }), (0,
            t.jsxs)("p", {
                className: "text-xs mt-4",
                children: [o("lblInputAuthOtp"), " ", (0,
                t.jsxs)("span", {
                    className: "text-red-600 text-[10px]",
                    children: [" ", "*", o("lblNoCopyPaste")]
                })]
            }), (0,
            t.jsx)("input", {
                type: "text",
                id: "otp",
                autoComplete: "off",
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-sm focus:ring-0 focus:border-slate-400 duration-300 border-[1.5px] focus:outline-0 block w-full p-2.5 my-2",
                placeholder: o("lblInputOtp"),
                inputMode: "numeric",
                value: p,
                maxLength: 6,
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault(),
                onChange: e => {
                    m(e.target.value.replace(/\D/g, ""))
                }
            }), (0,
            t.jsx)(r.motion.button, {
                type: "button",
                disabled: c || !x,
                whileTap: {
                    scale: .95
                },
                className: `text-white mb-2 mt-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition 
              ${c || !x ? "bg-gray-400 cursor-not-allowed" : "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838]"}`,
                onClick: !c && x ? D : void 0,
                children: o(c ? "checking" : "lblProceed")
            }), (0,
            t.jsx)(r.motion.button, {
                onClick: M,
                disabled: !k,
                className: `text-white mb-2 mt-2 md:ml-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition ${k ? "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838] cursor-pointer" : "bg-gray-400 cursor-not-allowed"}`,
                children: k ? o("resendOtp") : o("resendOtp") + ` ${w}s`
            })]
        })
    }
    ;
    var N = e.i(18566)
      , C = (0,
    i.createServerReference)("60a5f5e96fb267f718c17e98ee821eae0c3381977e", i.callServer, void 0, i.findSourceMapURL, "loginOtp")
      , k = (0,
    i.createServerReference)("6078c159defd71512496e894c041ec76dc18fe309b", i.callServer, void 0, i.findSourceMapURL, "submitPassword");
    let P = () => {
        let e = (0,
        N.useRouter)()
          , o = (0,
        s.useSelector)(e => e.authUser.user)
          , n = (0,
        s.useSelector)(e => e.ip.clientIp)
          , i = (0,
        s.useDispatch)()
          , c = (0,
        l.useTranslations)("root")
          , [m,h] = (0,
        a.useState)("")
          , [g,x] = (0,
        a.useState)(!1)
          , [b,f] = (0,
        a.useState)("")
          , y = "" !== m.trim() && m.length >= 6
          , [v,w] = (0,
        a.useState)(null)
          , [j,S] = (0,
        a.useState)(30)
          , [P,M] = (0,
        a.useState)(!0)
          , [D,I] = (0,
        a.useState)(!1);
        (0,
        a.useEffect)( () => {
            o.email && f(o.email)
        }
        , [o]);
        let T = async () => {
            try {
                x(!0);
                let t = await C({
                    mobile_no: o?.user_phone,
                    password: o?.password,
                    otp: m
                }, n || void 0);
                200 === t.status_code ? (i((0,
                u.setSlotAvailable)(t.data?.slot_available)),
                i((0,
                u.setAuthData)({
                    email: t?.data?.email,
                    user_phone: t?.data?.mobile_no,
                    name: t?.data?.name,
                    photo: t?.data?.profile_image
                })),
                i((0,
                u.clearPassword)()),
                e.push("/application")) : w({
                    message: t?.message,
                    type: "error"
                })
            } catch (a) {
                let e = "object" == typeof a && a?.message ? a.message : "Failed. Please try again later."
                  , t = "object" == typeof a && "number" == typeof a.status ? a.status : 422;
                (401 === t || 419 === t) && (w({
                    message: e,
                    type: "error"
                }),
                setTimeout( () => {
                    i((0,
                    d.setStep)(1))
                }
                , 5e3)),
                w({
                    message: e,
                    type: "error"
                })
            } finally {
                x(!1)
            }
        }
          , _ = async () => {
            x(!0);
            try {
                let e = await k({
                    mobile_no: o.user_phone,
                    password: o.password
                }, n || void 0);
                200 === e.status_code ? (S(30),
                M(!0),
                I(!1)) : (S(30),
                M(!0),
                I(!1),
                w({
                    message: e?.message,
                    type: "error"
                }))
            } catch (a) {
                let e = "object" == typeof a && a?.message ? a.message : "Failed. Please try again later."
                  , t = "object" == typeof a && "number" == typeof a.status ? a.status : 422;
                (401 === t || 419 === t) && (w({
                    message: e,
                    type: "error"
                }),
                setTimeout( () => {
                    i((0,
                    d.setStep)(1))
                }
                , 5e3)),
                w({
                    message: e,
                    type: "error"
                })
            } finally {
                x(!1)
            }
        }
        ;
        return (0,
        a.useEffect)( () => {
            let e;
            return P && j > 0 && (e = setInterval( () => {
                S(e => e - 1)
            }
            , 1e3)),
            0 === j && P && (clearInterval(e),
            M(!1),
            I(!0)),
            () => clearInterval(e)
        }
        , [j, P]),
        (0,
        t.jsxs)(r.motion.div, {
            className: "bg-white w-full p-4",
            variants: {
                hidden: {
                    opacity: 0,
                    y: 40
                },
                visible: {
                    opacity: 1,
                    y: 0,
                    transition: {
                        duration: .6,
                        ease: "easeOut"
                    }
                }
            },
            initial: "hidden",
            animate: "visible",
            children: [v && (0,
            t.jsx)(p.default, {
                message: v.message,
                type: v.type,
                onClose: () => w(null)
            }), (0,
            t.jsxs)("p", {
                className: "text-[10px]",
                children: [c("lblAuthOtp1"), " (", b, ")"]
            }), (0,
            t.jsx)("p", {
                className: "text-[10px]",
                children: c("lblAuthOtp2")
            }), (0,
            t.jsxs)("p", {
                className: "text-xs mt-4",
                children: [c("lblInputAuthOtp"), " ", (0,
                t.jsxs)("span", {
                    className: "text-red-600 text-[10px]",
                    children: [" ", "*", c("lblNoCopyPaste")]
                })]
            }), (0,
            t.jsx)("input", {
                type: "text",
                id: "otp",
                autoComplete: "off",
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-sm focus:ring-0 focus:border-slate-400 duration-300 border-[1.5px] focus:outline-0 block w-full p-2.5 my-2",
                placeholder: c("lblInputOtp"),
                inputMode: "numeric",
                value: m,
                maxLength: 6,
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault(),
                onChange: e => {
                    h(e.target.value.replace(/\D/g, ""))
                }
            }), (0,
            t.jsxs)("div", {
                children: [(0,
                t.jsx)(r.motion.button, {
                    type: "button",
                    whileTap: {
                        scale: .95
                    },
                    className: "text-white mb-2 mt-2 mr-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition bg-gray-400 hover:bg-[#218838] focus:ring-[#218838] cursor-pointer",
                    onClick: () => i((0,
                    d.setStep)(1)),
                    children: c("back")
                }), (0,
                t.jsx)(r.motion.button, {
                    type: "button",
                    disabled: g || !y,
                    whileTap: {
                        scale: .95
                    },
                    className: `text-white mb-2 mt-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition 
               ${g || !y ? "bg-gray-400 cursor-not-allowed" : "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838] cursor-pointer"}`,
                    onClick: y ? T : void 0,
                    children: g ? c("checking") + "..." : c("lblProceed")
                }), (0,
                t.jsx)(r.motion.button, {
                    onClick: _,
                    disabled: !D,
                    className: `text-white mb-2 mt-2 md:ml-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition ${D ? "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838] cursor-pointer" : "bg-gray-400 cursor-not-allowed"}`,
                    children: D ? c("resendOtp") : c("resendOtp") + ` ${j}s`
                })]
            })]
        })
    }
      , M = () => {
        let e = (0,
        s.useSelector)(e => e.authUser.user)
          , o = (0,
        s.useSelector)(e => e.ip.clientIp)
          , n = (0,
        s.useDispatch)()
          , i = (0,
        l.useTranslations)("root")
          , [c,m] = (0,
        a.useState)("")
          , [h,g] = (0,
        a.useState)(!1)
          , [x,b] = (0,
        a.useState)(null)
          , f = "" !== c.trim() && c.length >= 6
          , y = async () => {
            g(!0);
            try {
                let t = await k({
                    mobile_no: e?.user_phone,
                    password: c
                }, o || void 0);
                200 === t.status_code ? (n((0,
                u.setEmail)(t?.data?.email)),
                n((0,
                u.setPassword)(c)),
                n((0,
                d.setStep)(t.data?.email_confirmed ? 100 : 4))) : b({
                    message: t?.message,
                    type: "error"
                })
            } catch (a) {
                let e = "object" == typeof a && a?.message ? a.message : "Failed. Please try again later."
                  , t = "object" == typeof a && "number" == typeof a.status ? a.status : 422;
                (401 === t || 419 === t) && (b({
                    message: e,
                    type: "error"
                }),
                setTimeout( () => {
                    n((0,
                    d.setStep)(1))
                }
                , 5e3)),
                b({
                    message: e,
                    type: "error"
                })
            } finally {
                g(!1)
            }
        }
          , v = async () => {
            n((0,
            d.setStep)(7))
        }
        ;
        return (0,
        t.jsxs)(r.motion.div, {
            className: "bg-white w-full p-4",
            variants: {
                hidden: {
                    opacity: 0,
                    y: 40
                },
                visible: {
                    opacity: 1,
                    y: 0,
                    transition: {
                        duration: .6,
                        ease: "easeOut"
                    }
                }
            },
            initial: "hidden",
            animate: "visible",
            children: [x && (0,
            t.jsx)(p.default, {
                message: x.message,
                type: x.type,
                onClose: () => b(null)
            }), (0,
            t.jsxs)("p", {
                className: "text-xs mt-4",
                children: [i("lblInputPassword"), " ", (0,
                t.jsx)("span", {
                    className: "text-red-600 text-[10px]",
                    children: i("lblNoCopyPaste")
                })]
            }), (0,
            t.jsx)("input", {
                type: "password",
                id: "password",
                autoComplete: "off",
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault(),
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-sm focus:ring-0 focus:border-slate-400 duration-300 border-[1.5px] focus:outline-0 block w-full p-2.5 my-2",
                placeholder: i("lblInputPassword"),
                value: c,
                onChange: e => m(e.target.value)
            }), (0,
            t.jsxs)("div", {
                children: [(0,
                t.jsx)(r.motion.button, {
                    type: "button",
                    whileTap: {
                        scale: .95
                    },
                    className: "text-white mb-2 mt-2 mr-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition bg-gray-400 hover:bg-[#218838] focus:ring-[#218838] cursor-pointer",
                    onClick: () => n((0,
                    d.setStep)(1)),
                    children: i("back")
                }), (0,
                t.jsx)(r.motion.button, {
                    type: "button",
                    disabled: h || !f,
                    whileTap: {
                        scale: .95
                    },
                    className: `text-white mb-2 mt-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition 
         ${h || !f ? "bg-gray-400 cursor-not-allowed" : "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838] cursor-pointer"}`,
                    onClick: !h && f ? y : void 0,
                    children: i(h ? "checking" : "lblProceed")
                })]
            }), (0,
            t.jsx)("span", {
                className: "text-xs cursor-pointer text-blue-600 mt-2",
                onClick: v,
                children: i("lblInputForgotPassword") ?? "Forgot password ?"
            })]
        })
    }
    ;
    var D = (0,
    i.createServerReference)("602bec95e722f0976e4ed5ad8458785e476e19def5", i.callServer, void 0, i.findSourceMapURL, "registerUser")
      , I = (0,
    i.createServerReference)("60ab5557d866a9931849dc2d280c84259f087dc3ad", i.callServer, void 0, i.findSourceMapURL, "verifyEmail")
      , T = (0,
    i.createServerReference)("60cdc5240878841e0e831a792f7a9fc086d3f49188", i.callServer, void 0, i.findSourceMapURL, "verifyEmailOtp");
    let _ = ({message: e, type: s="info", onClose: l}) => {
        let[o,n] = (0,
        a.useState)(!0)
          , [i,c] = (0,
        a.useState)(!1)
          , [u,d] = (0,
        a.useState)(5);
        if ((0,
        a.useEffect)( () => {
            let e = null;
            return u > 0 ? e = setTimeout( () => d(e => e - 1), 1e3) : c(!0),
            () => {
                e && clearTimeout(e)
            }
        }
        , [u]),
        !o)
            return null;
        let p = (0,
        w.isHTML)(e) ? "Too many attempts. Please try again later." : e;
        return (0,
        t.jsx)(y.AnimatePresence, {
            children: o && (0,
            t.jsx)(r.motion.div, {
                initial: {
                    scale: .8,
                    opacity: 0
                },
                animate: {
                    scale: 1,
                    opacity: 1
                },
                exit: {
                    scale: .8,
                    opacity: 0
                },
                transition: {
                    type: "spring",
                    stiffness: 150,
                    damping: 25
                },
                className: "fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm",
                children: (0,
                t.jsxs)("div", {
                    className: `flex justify-center flex-col h-1/2 rounded-xl shadow-2xl p-6 max-w-md w-full mx-4 text-center ${{
                        success: "bg-green-500 text-white",
                        error: "bg-red-500 text-white",
                        info: "bg-blue-500 text-white",
                        warning: "bg-yellow-400 text-black"
                    }[s]}`,
                    children: [(0,
                    t.jsxs)("div", {
                        className: "flex flex-col justify-center items-center mb-4",
                        children: [(0,
                        t.jsx)(v.CloseIcon, {}), (0,
                        t.jsx)("p", {
                            className: "text-white font-bold text-2xl py-4",
                            children: "Error!"
                        })]
                    }), (0,
                    t.jsx)("span", {
                        className: "block mb-4",
                        children: p
                    }), i ? (0,
                    t.jsx)("button", {
                        onClick: () => {
                            n(!1),
                            l?.()
                        }
                        ,
                        className: "mt-2 w-24 py-2 rounded bg-gray-500 text-white hover:bg-gray-600 mx-auto",
                        children: "OK"
                    }) : (0,
                    t.jsxs)("p", {
                        className: "text-sm text-white",
                        children: ["Please wait ", u, " seconds..."]
                    })]
                })
            })
        })
    }
      , O = () => {
        let e = (0,
        s.useSelector)(e => e.authUser.user)
          , o = (0,
        s.useSelector)(e => e.ip.clientIp)
          , n = (0,
        s.useDispatch)()
          , i = (0,
        l.useTranslations)("root")
          , [c,u] = (0,
        a.useState)(null)
          , [m,h] = (0,
        a.useState)(null)
          , [g,x] = (0,
        a.useState)("")
          , [b,f] = (0,
        a.useState)(null)
          , [y,v] = (0,
        a.useState)(null)
          , [w,j] = (0,
        a.useState)("")
          , [S,N] = (0,
        a.useState)("")
          , [C,k] = (0,
        a.useState)("")
          , [P,M] = (0,
        a.useState)("")
          , [O,R] = (0,
        a.useState)("")
          , [$,A] = (0,
        a.useState)(!1)
          , [E,L] = (0,
        a.useState)(!1)
          , [U,F] = (0,
        a.useState)(!1)
          , [z,V] = (0,
        a.useState)(!1)
          , [B,H] = (0,
        a.useState)(!1)
          , [K,G] = (0,
        a.useState)(0)
          , [J,q] = (0,
        a.useState)(!1)
          , X = (0,
        a.useRef)(null)
          , Y = e => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e.trim())
          , W = b && g.length >= 11 && "" !== w.trim() && Y(S) && "" !== P.trim() && "" !== O.trim() && P === O
          , Q = async () => {
            try {
                H(!0);
                let e = new FormData;
                e.append("photo", b),
                e.append("mobile_no", g),
                e.append("full_name", w),
                e.append("email", S),
                e.append("password", P),
                e.append("password_confirmation", O),
                e.append("resend", "0");
                let t = await D(e, o || void 0);
                t?.status_code === 200 ? n((0,
                d.setStep)(6)) : u({
                    message: t?.message || "Registration failed",
                    type: "error"
                })
            } catch (e) {
                u({
                    message: e?.message || "Failed. Please try again later.",
                    type: "error"
                })
            } finally {
                H(!1)
            }
        }
          , Z = "" !== S.trim() && Y(S)
          , ee = async () => {
            try {
                F(!0);
                let e = await I({
                    email: S
                }, o || void 0);
                200 === e.status_code ? A(!0) : u({
                    message: e?.message,
                    type: "error"
                })
            } catch (e) {
                u({
                    message: "Failed. Please try again.",
                    type: "error"
                })
            } finally {
                F(!1)
            }
        }
          , et = "" !== C.trim() && C.length >= 6
          , ea = async () => {
            try {
                V(!0);
                let e = await T({
                    email: S,
                    otp: C
                }, o || void 0);
                200 === e.status_code ? L(!0) : u({
                    message: e?.message,
                    type: "error"
                })
            } catch (e) {
                u({
                    message: "OTP verification failed",
                    type: "error"
                })
            } finally {
                V(!1)
            }
        }
        ;
        (0,
        a.useEffect)( () => {
            let e = null;
            return J && K > 0 ? e = setInterval( () => G(e => e - 1), 1e3) : 0 === K && q(!1),
            () => clearInterval(e)
        }
        , [K, J]);
        let es = () => {
            ee(),
            G(30),
            q(!0)
        }
        ;
        return (0,
        a.useEffect)( () => {
            e?.user_phone && x(e.user_phone)
        }
        , [e]),
        (0,
        t.jsxs)(r.motion.div, {
            className: "bg-white w-full p-4",
            variants: {
                hidden: {
                    opacity: 0,
                    y: 40
                },
                visible: {
                    opacity: 1,
                    y: 0,
                    transition: {
                        duration: .6,
                        ease: "easeOut"
                    }
                }
            },
            initial: "hidden",
            animate: "visible",
            children: [c && (0,
            t.jsx)(p.default, {
                message: c.message,
                type: c.type,
                onClose: () => u(null)
            }), m && (0,
            t.jsx)(_, {
                message: m.message,
                type: m.type,
                onClose: () => h(null)
            }), (0,
            t.jsxs)("p", {
                className: "text-xs mb-2 uppercase",
                children: [i("lblInputRegPhotoUpload"), " ", (0,
                t.jsxs)("span", {
                    className: "text-red-600 text-[10px]",
                    children: ["*", i("lblInputRegPhotoSize")]
                })]
            }), (0,
            t.jsx)("div", {
                className: "relative w-24 h-24 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-gray-400 transition-colors flex items-center justify-center overflow-hidden",
                onClick: () => X.current?.click(),
                children: b ? (0,
                t.jsx)("img", {
                    src: y || "/placeholder.svg",
                    className: "w-full h-full object-cover"
                }) : (0,
                t.jsx)("svg", {
                    className: "w-16 h-16 text-gray-300",
                    fill: "currentColor",
                    viewBox: "0 0 24 24",
                    children: (0,
                    t.jsx)("path", {
                        d: "M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"
                    })
                })
            }), (0,
            t.jsx)("input", {
                ref: X,
                type: "file",
                accept: "image/*",
                onChange: e => {
                    let t = e.target.files?.[0];
                    t && (t.size > 2097152 ? u({
                        message: "File size must be less than 2MB",
                        type: "error"
                    }) : ["image/jpeg", "image/jpg", "image/png"].includes(t.type) ? (f(t),
                    v(URL.createObjectURL(t))) : u({
                        message: "Please select a JPG, PNG, or JPEG file",
                        type: "error"
                    }))
                }
                ,
                className: "hidden"
            }), (0,
            t.jsx)("p", {
                className: "text-xs mt-4 uppercase",
                children: i("lblInputRegMobile")
            }), (0,
            t.jsx)("input", {
                type: "tel",
                id: "phone",
                autoComplete: "off",
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault(),
                disabled: !0,
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-lg focus:ring-0 focus:border-blue-400 focus:border-[0.1px] border-[0.5px] block w-full p-2.5 my-2",
                placeholder: i("lblInputRegMobile"),
                inputMode: "numeric",
                pattern: "[0-9]*",
                maxLength: 11,
                value: g,
                onChange: e => x(e.target.value)
            }), (0,
            t.jsx)("p", {
                className: "text-xs mt-4 uppercase",
                children: i("lblInputRegName")
            }), (0,
            t.jsx)("input", {
                type: "text",
                id: "name",
                autoComplete: "off",
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault(),
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-lg focus:ring-0 focus:border-black focus:border-[0.5px] border-[0.5px] block w-full p-2.5 my-2",
                placeholder: i("lblInputRegName"),
                onChange: e => j(e.target.value),
                value: w
            }), (0,
            t.jsx)("p", {
                className: "text-xs mt-4 uppercase",
                children: i("lblInputEmail")
            }), (0,
            t.jsx)("input", {
                type: "email",
                id: "email",
                autoComplete: "off",
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault(),
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-lg focus:ring-0 focus:border-black focus:border-[0.5px] border-[0.5px] block w-full p-2.5 my-2",
                placeholder: i("lblInputEmail"),
                onChange: e => N(e.target.value),
                value: S
            }), E && (0,
            t.jsx)("p", {
                className: "text-xs text-green-500 mt-1",
                children: i("emailSuccessfullyVerified")
            }), !$ && (0,
            t.jsx)("button", {
                type: "button",
                disabled: !Z || U,
                onClick: Z ? es : void 0,
                className: `text-xs font-medium rounded px-2 py-1.5 mt-2
            ${!Z ? "bg-gray-300 text-gray-400" : "bg-blue-700 text-white"}`,
                children: i(U ? "checking" : "verify_email")
            }), $ && !E && (0,
            t.jsx)("button", {
                onClick: es,
                disabled: J,
                className: `text-xs font-medium rounded px-2 py-1.5 mt-2
            ${J ? "bg-gray-300 text-gray-400" : "bg-blue-700 text-white"}`,
                children: J ? `${i("resendOtp")} ${K}s` : i("resendOtp")
            }), $ && !E && (0,
            t.jsxs)(t.Fragment, {
                children: [(0,
                t.jsx)("input", {
                    type: "text",
                    id: "otp",
                    autoComplete: "off",
                    onCopy: e => e.preventDefault(),
                    onPaste: e => e.preventDefault(),
                    onCut: e => e.preventDefault(),
                    className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-lg focus:ring-0 focus:border-black focus:border-[0.5px] border-[0.5px] block w-full p-2.5 my-2",
                    placeholder: "Enter OTP",
                    maxLength: 6,
                    onChange: e => k(e.target.value),
                    value: C
                }), (0,
                t.jsx)("button", {
                    type: "button",
                    disabled: !et || z,
                    onClick: et ? ea : void 0,
                    className: `text-xs font-medium rounded px-2 py-1.5 mt-2
              ${!et ? "bg-gray-300 text-gray-400" : "bg-blue-700 text-white"}`,
                    children: i(z ? "checking" : "verifyOtp")
                })]
            }), (0,
            t.jsx)("p", {
                className: "text-xs mt-4",
                children: i("lblInputPassword")
            }), (0,
            t.jsx)("input", {
                type: "password",
                id: "password",
                disabled: !E,
                autoComplete: "off",
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault(),
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-lg focus:ring-0 focus:border-black focus:border-[0.5px] border-[0.5px] block w-full p-2.5 my-2",
                placeholder: "********",
                onChange: e => M(e.target.value),
                value: P
            }), (0,
            t.jsx)("p", {
                className: "text-xs mt-4",
                children: i("lblInputRePassword")
            }), (0,
            t.jsx)("input", {
                type: "password",
                id: "confirmPassword",
                autoComplete: "off",
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault(),
                disabled: !E,
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-lg focus:ring-0 focus:border-black focus:border-[0.5px] border-[0.5px] block w-full p-2.5 my-2",
                placeholder: "********",
                onChange: e => R(e.target.value),
                value: O
            }), (0,
            t.jsx)(r.motion.button, {
                type: "button",
                disabled: !W || B,
                whileTap: {
                    scale: .95
                },
                onClick: W ? Q : void 0,
                className: `text-white mb-2 mt-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition 
            ${!W || B ? "bg-gray-400 cursor-not-allowed" : "bg-[#28a745] hover:bg-[#218838]"}
            `,
                children: i(B ? "checking" : "lblProceed")
            })]
        })
    }
    ;
    var R = (0,
    i.createServerReference)("605a28da919571e850a9cda6fbb6fde3eafea0b85e", i.callServer, void 0, i.findSourceMapURL, "changePassword");
    let $ = () => {
        let e = (0,
        s.useDispatch)()
          , o = (0,
        l.useTranslations)("root")
          , n = (0,
        s.useSelector)(e => e.ip.clientIp)
          , [i,c] = (0,
        a.useState)("")
          , [m,h] = (0,
        a.useState)(!1)
          , [g,x] = (0,
        a.useState)(null)
          , b = "" !== i.trim() && i.length >= 11
          , f = async () => {
            h(!0);
            try {
                b = !1;
                let t = await R({
                    mobile_no: i
                }, n || void 0);
                200 === t.status_code ? (e((0,
                u.setPhoneNumber)(i)),
                e((0,
                u.setEmail)(t.data?.email)),
                e((0,
                d.setStep)(t.data?.email_confirmed ? 9 : 8))) : x({
                    message: t?.message,
                    type: "error"
                })
            } catch (s) {
                let t = "object" == typeof s && s?.message ? s.message : "Failed. Please try again later."
                  , a = "object" == typeof s && "number" == typeof s.status ? s.status : 422;
                (401 === a || 419 === a) && (x({
                    message: t,
                    type: "error"
                }),
                setTimeout( () => {
                    e((0,
                    d.setStep)(1))
                }
                , 5e3)),
                x({
                    message: t,
                    type: "error"
                })
            } finally {
                h(!1)
            }
        }
        ;
        return (0,
        t.jsxs)(r.motion.div, {
            className: "bg-white w-full p-4",
            variants: {
                hidden: {
                    opacity: 0,
                    y: 40
                },
                visible: {
                    opacity: 1,
                    y: 0,
                    transition: {
                        duration: .6,
                        ease: "easeOut"
                    }
                }
            },
            initial: "hidden",
            animate: "visible",
            children: [g && (0,
            t.jsx)(p.default, {
                message: g.message,
                type: g.type,
                onClose: () => x(null)
            }), (0,
            t.jsxs)("p", {
                className: "text-xs mt-2 mb-2",
                children: [o("lblInputLoginMobilePassChange"), " ", (0,
                t.jsx)("span", {
                    className: "text-red-600 text-[10px]",
                    children: o("lblInputLoginMobileMsg")
                })]
            }), (0,
            t.jsx)("input", {
                type: "tel",
                id: "phone",
                autoComplete: "off",
                className: " bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-sm focus:ring-0 focus:border-slate-400 duration-300 border-[1.5px] focus:outline-0 block w-full p-2.5 mb-3",
                placeholder: o("lblInputLoginMobile"),
                inputMode: "numeric",
                pattern: "[0-9]*",
                maxLength: 11,
                value: i,
                onChange: e => {
                    c(e.target.value.replace(/\D/g, ""))
                }
            }), (0,
            t.jsx)(r.motion.button, {
                type: "button",
                disabled: m || !b,
                whileTap: {
                    scale: .95
                },
                whileHover: {
                    scale: !m && b ? 1.03 : 1
                },
                className: `text-white mb-2 h-8 rounded-lg  text-xs w-full duration-300 sm:w-auto px-2 py-2 text-center transition 
          ${m || !b ? "bg-gray-400 cursor-not-allowed" : "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838] cursor-pointer"}`,
                onClick: !m && b ? f : void 0,
                children: o(m ? "checking" : "lblProceed")
            })]
        })
    }
    ;
    var A = e.i(42222)
      , E = (0,
    i.createServerReference)("605693b672675b4a33c68d5bd9de5d6df49891a706", i.callServer, void 0, i.findSourceMapURL, "changePasswordOtp");
    let L = () => {
        let e = (0,
        s.useDispatch)()
          , o = (0,
        A.useAppSelector)(e => e.authUser.user)
          , n = (0,
        l.useTranslations)("root")
          , i = (0,
        s.useSelector)(e => e.ip.clientIp)
          , [c,m] = (0,
        a.useState)("")
          , [h,g] = (0,
        a.useState)(!1)
          , [x,b] = (0,
        a.useState)("")
          , [f,y] = (0,
        a.useState)(null)
          , [v,w] = (0,
        a.useState)(30)
          , [j,S] = (0,
        a.useState)(!0)
          , [N,C] = (0,
        a.useState)(!1)
          , k = "" !== c.trim() && c.length >= 6
          , P = async () => {
            g(!0);
            try {
                let t = await E({
                    mobile_no: o?.user_phone,
                    password: o?.password,
                    otp: c
                }, i || void 0);
                200 === t.status_code ? (e((0,
                u.setName)(t?.data?.name)),
                e((0,
                u.setEmail)(t?.data?.email)),
                e((0,
                u.setPhoneNumber)(t?.data?.mobile_no)),
                e((0,
                u.setPhoto)(t?.data?.profile_image)),
                e((0,
                d.setStep)(10))) : y({
                    message: t?.message,
                    type: "error"
                })
            } catch (s) {
                let t = "object" == typeof s && s?.message ? s.message : "Failed. Please try again later."
                  , a = "object" == typeof s && "number" == typeof s.status ? s.status : 422;
                (401 === a || 419 === a) && (y({
                    message: t,
                    type: "error"
                }),
                setTimeout( () => {
                    e((0,
                    d.setStep)(1))
                }
                , 5e3)),
                y({
                    message: t,
                    type: "error"
                })
            } finally {
                g(!1)
            }
        }
          , M = async () => {
            g(!0);
            try {
                let e = localStorage.getItem("user_phone")
                  , t = await R({
                    mobile_no: e
                }, i || void 0);
                200 === t.status_code ? (w(30),
                S(!0),
                C(!1)) : (w(30),
                S(!0),
                C(!1),
                y({
                    message: t?.message,
                    type: "error"
                }))
            } catch (s) {
                let t = "object" == typeof s && s?.message ? s.message : "Failed. Please try again later."
                  , a = "object" == typeof s && "number" == typeof s.status ? s.status : 422;
                (401 === a || 419 === a) && (y({
                    message: t,
                    type: "error"
                }),
                setTimeout( () => {
                    e((0,
                    d.setStep)(1))
                }
                , 5e3)),
                y({
                    message: t,
                    type: "error"
                })
            } finally {
                g(!1)
            }
        }
        ;
        return (0,
        a.useEffect)( () => {
            o.email && b(o.email)
        }
        , [o]),
        (0,
        a.useEffect)( () => {
            let e;
            return j && v > 0 && (e = setInterval( () => {
                w(e => e - 1)
            }
            , 1e3)),
            0 === v && j && (clearInterval(e),
            S(!1),
            C(!0)),
            () => clearInterval(e)
        }
        , [v, j]),
        (0,
        t.jsxs)(r.motion.div, {
            className: "bg-white w-full p-4",
            variants: {
                hidden: {
                    opacity: 0,
                    y: 40
                },
                visible: {
                    opacity: 1,
                    y: 0,
                    transition: {
                        duration: .6,
                        ease: "easeOut"
                    }
                }
            },
            initial: "hidden",
            animate: "visible",
            children: [f && (0,
            t.jsx)(p.default, {
                message: f.message,
                type: f.type,
                onClose: () => y(null)
            }), (0,
            t.jsxs)("p", {
                className: "text-[10px]",
                children: [n("lblAuthOtp1"), " (", x, ")"]
            }), (0,
            t.jsx)("p", {
                className: "text-[10px]",
                children: n("lblAuthOtp2")
            }), (0,
            t.jsxs)("p", {
                className: "text-xs mt-4",
                children: [n("lblInputAuthOtp"), " ", (0,
                t.jsxs)("span", {
                    className: "text-red-600 text-[10px]",
                    children: [" ", "*", n("lblNoCopyPaste")]
                })]
            }), (0,
            t.jsx)("input", {
                type: "text",
                id: "otp",
                autoComplete: "off",
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-lg focus:ring-0 focus:border-blue-400 focus:border-[0.1px] border-[0.5px] block w-full p-2.5 my-2",
                placeholder: n("lblInputOtp"),
                inputMode: "numeric",
                value: c,
                maxLength: 6,
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault(),
                onChange: e => {
                    m(e.target.value.replace(/\D/g, ""))
                }
            }), (0,
            t.jsx)(r.motion.button, {
                type: "button",
                disabled: h || !k,
                whileTap: {
                    scale: .95
                },
                className: `text-white mb-2 mt-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition 
               ${h || !k ? "bg-gray-400 cursor-not-allowed" : "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838] cursor-pointer"}`,
                onClick: k ? P : void 0,
                children: h ? n("checking") + "..." : n("lblProceed")
            }), (0,
            t.jsx)(r.motion.button, {
                onClick: M,
                disabled: !N,
                className: `text-white mb-2 mt-2 md:ml-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition ${N ? "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838] cursor-pointer" : "bg-gray-400 cursor-not-allowed"}`,
                children: N ? n("resendOtp") : n("resendOtp") + ` ${v}s`
            })]
        })
    }
    ;
    var U = (0,
    i.createServerReference)("607eae7598d1ed12dee8343a3eff5b4f9a22f4821f", i.callServer, void 0, i.findSourceMapURL, "changePasswordConfirm");
    let F = () => {
        let e = (0,
        l.useTranslations)("root")
          , o = (0,
        s.useDispatch)()
          , n = (0,
        s.useSelector)(e => e.ip.clientIp)
          , [i,c] = (0,
        a.useState)("")
          , [p,m] = (0,
        a.useState)(!1)
          , [h,g] = (0,
        a.useState)("")
          , [x,b] = (0,
        a.useState)(null)
          , f = "" !== i.trim() && i.length >= 6 && "" !== h.trim() && h.length >= 6
          , y = async () => {
            try {
                if (m(!0),
                i !== h)
                    return void b({
                        message: "Password and confirm password do not match",
                        type: "error"
                    });
                let e = await U({
                    password: i,
                    password_confirmation: h
                }, n || void 0);
                200 === e.status_code ? (o((0,
                u.clearAll)()),
                b({
                    message: "Password changed successfully",
                    type: "success"
                })) : b({
                    message: e?.message,
                    type: "error"
                })
            } catch (a) {
                let e = "object" == typeof a && a?.message ? a.message : "Failed. Please try again later."
                  , t = "object" == typeof a && "number" == typeof a.status ? a.status : 422;
                (401 === t || 419 === t) && (b({
                    message: e,
                    type: "error"
                }),
                setTimeout( () => {
                    o((0,
                    d.setStep)(1))
                }
                , 5e3)),
                b({
                    message: e,
                    type: "error"
                })
            } finally {
                m(!1)
            }
        }
        ;
        return (0,
        t.jsxs)(r.motion.div, {
            className: "bg-white w-full p-4",
            variants: {
                hidden: {
                    opacity: 0,
                    y: 40
                },
                visible: {
                    opacity: 1,
                    y: 0,
                    transition: {
                        duration: .6,
                        ease: "easeOut"
                    }
                }
            },
            initial: "hidden",
            animate: "visible",
            children: [x && (0,
            t.jsx)(j, {
                message: x.message,
                type: x.type,
                onClose: () => b(null)
            }), (0,
            t.jsxs)("p", {
                className: "text-xs mt-4",
                children: [e("lblInputPassword"), " ", (0,
                t.jsx)("span", {
                    className: "text-red-600 text-[10px]",
                    children: e("lblNoCopyPaste")
                })]
            }), (0,
            t.jsx)("input", {
                type: "password",
                id: "password",
                autoComplete: "off",
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault(),
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-lg focus:ring-0 focus:border-blue-400 focus:border-[0.1px] border-[0.5px] block w-full p-2.5 my-2",
                placeholder: e("lblInputPassword"),
                value: i,
                onChange: e => c(e.target.value)
            }), i.length > 0 && i.length < 6 && (0,
            t.jsx)("p", {
                className: "text-red-500 text-[11px] mb-2",
                children: e("passwordMinLengthMsg")
            }), (0,
            t.jsxs)("p", {
                className: "text-xs mt-4",
                children: [e("lblInputRePassword"), " ", (0,
                t.jsx)("span", {
                    className: "text-red-600 text-[10px]",
                    children: e("lblNoCopyPaste")
                })]
            }), (0,
            t.jsx)("input", {
                type: "password",
                id: "password_confirm",
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault(),
                autoComplete: "off",
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-lg focus:ring-0 focus:border-blue-400 focus:border-[0.1px] border-[0.5px] block w-full p-2.5 my-2",
                placeholder: e("lblInputRePassword"),
                value: h,
                onChange: e => g(e.target.value)
            }), (0,
            t.jsx)(r.motion.button, {
                type: "button",
                disabled: !f,
                whileTap: {
                    scale: .95
                },
                className: `text-white mb-2 mt-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition 
         ${p || !f ? "bg-gray-400 cursor-not-allowed" : "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838] cursor-pointer"}`,
                onClick: !p && f ? y : void 0,
                children: p ? e("checking") + "..." : e("lblProceed")
            })]
        })
    }
    ;
    var z = (0,
    i.createServerReference)("60f402535c61cd17a63d48207038aa2961c12f932f", i.callServer, void 0, i.findSourceMapURL, "changePasswordEmailVerify");
    let V = () => {
        let e = (0,
        s.useDispatch)()
          , o = (0,
        A.useAppSelector)(e => e.authUser.user)
          , n = (0,
        l.useTranslations)("root")
          , i = (0,
        s.useSelector)(e => e.ip.clientIp)
          , [c,u] = (0,
        a.useState)("")
          , [m,h] = (0,
        a.useState)("")
          , [g,x] = (0,
        a.useState)(null)
          , [b,f] = (0,
        a.useState)(!1)
          , y = "" !== c.trim() && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(c.trim());
        (0,
        a.useEffect)( () => {
            if (o.email)
                try {
                    let e = JSON.parse(o.email);
                    h(e)
                } catch {
                    h(o.email)
                }
        }
        , [o]);
        let v = async () => {
            try {
                f(!0);
                let t = await z({
                    email: c
                }, i || void 0);
                200 === t.status_code ? e((0,
                d.setStep)(9)) : x({
                    message: t?.message,
                    type: "error"
                })
            } catch (s) {
                let t = "object" == typeof s && s?.message ? s.message : "Failed. Please try again later."
                  , a = "object" == typeof s && "number" == typeof s.status ? s.status : 422;
                (401 === a || 419 === a) && (x({
                    message: t,
                    type: "error"
                }),
                setTimeout( () => {
                    e((0,
                    d.setStep)(1))
                }
                , 5e3)),
                x({
                    message: t,
                    type: "error"
                })
            } finally {
                f(!1)
            }
        }
        ;
        return (0,
        t.jsxs)(r.motion.div, {
            className: "bg-white w-full p-4",
            variants: {
                hidden: {
                    opacity: 0,
                    y: 40
                },
                visible: {
                    opacity: 1,
                    y: 0,
                    transition: {
                        duration: .6,
                        ease: "easeOut"
                    }
                }
            },
            initial: "hidden",
            animate: "visible",
            children: [g && (0,
            t.jsx)(p.default, {
                message: g.message,
                type: g.type,
                onClose: () => x(null)
            }), (0,
            t.jsxs)("p", {
                className: "text-xs",
                children: [n("lblAuthFullEmail"), " (", m, ")"]
            }), (0,
            t.jsx)("p", {
                className: "text-xs my-1",
                children: n("lblAuthFullEmail2")
            }), (0,
            t.jsxs)("p", {
                className: "text-xs mt-4",
                children: [n("lblInputRegEmail"), " ", (0,
                t.jsxs)("span", {
                    className: "text-red-600 text-[10px]",
                    children: ["*", n("lblNoCopyPaste")]
                })]
            }), (0,
            t.jsx)("input", {
                type: "email",
                id: "phone",
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-lg focus:ring-0 focus:border-black focus:border-[0.5px] border-[0.5px] block w-full p-2.5 my-2",
                placeholder: n("lblInputRegEmail"),
                value: c,
                onChange: e => u(e.target.value),
                autoComplete: "off",
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault()
            }), (0,
            t.jsx)(r.motion.button, {
                type: "button",
                disabled: !y || b,
                whileTap: {
                    scale: .95
                },
                whileHover: {
                    scale: !b && y ? 1.03 : 1
                },
                className: `text-white mb-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition 
              ${!y || b ? "bg-gray-400 cursor-not-allowed" : "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838]"}`,
                onClick: !b && y ? v : void 0,
                children: b ? n("checking") + "..." : n("lblProceed")
            })]
        })
    }
    ;
    var B = (0,
    i.createServerReference)("609028e470692c51258daa1bdc74a1b0c6487be0ad", i.callServer, void 0, i.findSourceMapURL, "loginEmailVerify");
    let H = () => {
        let e = (0,
        s.useDispatch)()
          , o = (0,
        l.useTranslations)("root")
          , n = (0,
        s.useSelector)(e => e.authUser.user)
          , i = (0,
        s.useSelector)(e => e.ip.clientIp)
          , [c,u] = (0,
        a.useState)("")
          , [m] = (0,
        a.useState)(( () => {
            if (n.email)
                try {
                    return JSON.parse(n.email)
                } catch {
                    return n.email
                }
            return ""
        }
        )())
          , [h,g] = (0,
        a.useState)(null)
          , x = "" !== c.trim() && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(c.trim())
          , b = async () => {
            try {
                let t = await B({
                    email: c
                }, i || void 0);
                200 === t.status_code ? e((0,
                d.setStep)(5)) : g({
                    message: t?.message,
                    type: "error"
                })
            } catch (s) {
                let t = "object" == typeof s && s?.message ? s.message : "Failed. Please try again later."
                  , a = "object" == typeof s && "number" == typeof s.status ? s.status : 422;
                (401 === a || 419 === a) && (g({
                    message: t,
                    type: "error"
                }),
                setTimeout( () => {
                    e((0,
                    d.setStep)(1))
                }
                , 5e3)),
                g({
                    message: t,
                    type: "error"
                })
            }
        }
        ;
        return (0,
        t.jsxs)(r.motion.div, {
            className: "bg-white w-full p-4",
            variants: {
                hidden: {
                    opacity: 0,
                    y: 40
                },
                visible: {
                    opacity: 1,
                    y: 0,
                    transition: {
                        duration: .6,
                        ease: "easeOut"
                    }
                }
            },
            initial: "hidden",
            animate: "visible",
            children: [h && (0,
            t.jsx)(p.default, {
                message: h.message,
                type: h.type,
                onClose: () => g(null)
            }), (0,
            t.jsxs)("p", {
                className: "text-xs",
                children: [o("lblAuthFullEmail"), " (", m, ")"]
            }), (0,
            t.jsx)("p", {
                className: "text-xs my-1",
                children: o("lblAuthFullEmail2")
            }), (0,
            t.jsxs)("p", {
                className: "text-xs mt-4",
                children: [o("lblInputRegEmail"), " ", (0,
                t.jsxs)("span", {
                    className: "text-red-600 text-[10px]",
                    children: ["*", o("lblNoCopyPaste")]
                })]
            }), (0,
            t.jsx)("input", {
                type: "email",
                id: "phone",
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-lg focus:ring-0 focus:border-black focus:border-[0.5px] border-[0.5px] block w-full p-2.5 my-2",
                placeholder: o("lblInputRegEmail"),
                value: c,
                onChange: e => u(e.target.value),
                autoComplete: "off",
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault()
            }), (0,
            t.jsx)(r.motion.button, {
                type: "button",
                disabled: !x,
                whileTap: {
                    scale: .95
                },
                whileHover: {
                    scale: x ? 1.03 : 1
                },
                className: `text-white mb-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition 
        ${x ? "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838]" : "bg-gray-400 cursor-not-allowed"}`,
                onClick: x ? b : void 0,
                children: o("lblProceed")
            })]
        })
    }
    ;
    var K = (0,
    i.createServerReference)("60f4fb45ca4da6f7253c4a73a7b81c8e57f6c0c6e9", i.callServer, void 0, i.findSourceMapURL, "loginEmailOtpVerify");
    let G = () => {
        let e = (0,
        N.useRouter)()
          , o = (0,
        s.useDispatch)()
          , n = (0,
        l.useTranslations)("root")
          , i = (0,
        s.useSelector)(e => e.ip.clientIp)
          , [c,m] = (0,
        a.useState)(!1)
          , [h,g] = (0,
        a.useState)("")
          , [x,b] = (0,
        a.useState)("")
          , f = "" !== h.trim() && h.length >= 6
          , [y,v] = (0,
        a.useState)(null)
          , [w,j] = (0,
        a.useState)(30)
          , [S,C] = (0,
        a.useState)(!0)
          , [k,P] = (0,
        a.useState)(!1);
        (0,
        a.useEffect)( () => {
            let e = localStorage.getItem("user_email");
            e && b(e)
        }
        , []);
        let M = async () => {
            m(!0);
            try {
                let e = await B({
                    email: x
                }, i || void 0);
                200 === e.status_code ? o((0,
                d.setStep)(5)) : v({
                    message: e?.message,
                    type: "error"
                })
            } catch (a) {
                let e = "object" == typeof a && a?.message ? a.message : "Failed. Please try again later."
                  , t = "object" == typeof a && "number" == typeof a.status ? a.status : 422;
                (401 === t || 419 === t) && (v({
                    message: e,
                    type: "error"
                }),
                setTimeout( () => {
                    o((0,
                    d.setStep)(1))
                }
                , 5e3)),
                v({
                    message: e,
                    type: "error"
                })
            }
        }
        ;
        (0,
        a.useEffect)( () => {
            let e;
            return S && w > 0 && (e = setInterval( () => {
                j(e => e - 1)
            }
            , 1e3)),
            0 === w && S && (clearInterval(e),
            C(!1),
            P(!0)),
            () => clearInterval(e)
        }
        , [w, S]);
        let D = async () => {
            try {
                m(!0);
                let t = await K({
                    otp: h
                }, i || void 0);
                if (200 === t.status_code) {
                    if (t?.data?.access_token) {
                        let e = new Date;
                        e.setDate(e.getDate() + 7),
                        document.cookie = `access_token=${t.data.access_token}; expires=${e.toUTCString()}; path=/`
                    }
                    o((0,
                    u.setSlotAvailable)(t.data?.slot_available)),
                    o((0,
                    u.setAuthData)({
                        email: t?.data?.email,
                        user_phone: t?.data?.mobile_no,
                        name: t?.data?.name,
                        photo: t?.data?.profile_image
                    })),
                    o((0,
                    u.clearPassword)()),
                    e.push("/application")
                } else
                    v({
                        message: t?.message,
                        type: "error"
                    })
            } catch (a) {
                let e = "object" == typeof a && a?.message ? a.message : "Failed. Please try again later."
                  , t = "object" == typeof a && "number" == typeof a.status ? a.status : 422;
                (401 === t || 419 === t) && (v({
                    message: e,
                    type: "error"
                }),
                setTimeout( () => {
                    o((0,
                    d.setStep)(1))
                }
                , 5e3)),
                v({
                    message: e,
                    type: "error"
                })
            } finally {
                m(!1)
            }
        }
        ;
        return (0,
        t.jsxs)(r.motion.div, {
            className: "bg-white w-full p-4",
            variants: {
                hidden: {
                    opacity: 0,
                    y: 40
                },
                visible: {
                    opacity: 1,
                    y: 0,
                    transition: {
                        duration: .6,
                        ease: "easeOut"
                    }
                }
            },
            initial: "hidden",
            animate: "visible",
            children: [y && (0,
            t.jsx)(p.default, {
                message: y.message,
                type: y.type,
                onClose: () => v(null)
            }), (0,
            t.jsxs)("p", {
                className: "text-[10px]",
                children: [n("lblAuthOtp1"), " (", x, ")"]
            }), (0,
            t.jsx)("p", {
                className: "text-[10px]",
                children: n("lblAuthOtp2")
            }), (0,
            t.jsxs)("p", {
                className: "text-xs mt-4",
                children: [n("lblInputAuthOtp"), " ", (0,
                t.jsxs)("span", {
                    className: "text-red-600 text-[10px]",
                    children: [" ", "*", n("lblNoCopyPaste")]
                })]
            }), (0,
            t.jsx)("input", {
                type: "text",
                id: "otp",
                autoComplete: "off",
                className: "bg-white border-gray-300 h-8 text-gray-900 font-light text-xs rounded-sm focus:ring-0 focus:border-slate-400 duration-300 border-[1.5px] focus:outline-0 block w-full p-2.5 my-2",
                placeholder: n("lblInputOtp"),
                inputMode: "numeric",
                value: h,
                maxLength: 6,
                onCopy: e => e.preventDefault(),
                onPaste: e => e.preventDefault(),
                onCut: e => e.preventDefault(),
                onChange: e => {
                    g(e.target.value.replace(/\D/g, ""))
                }
            }), (0,
            t.jsxs)("div", {
                children: [(0,
                t.jsx)(r.motion.button, {
                    type: "button",
                    disabled: c || !f,
                    whileTap: {
                        scale: .95
                    },
                    className: `text-white mb-2 mt-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition 
              ${c || !f ? "bg-gray-400 cursor-not-allowed" : "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838]"}`,
                    onClick: !c && f ? D : void 0,
                    children: c ? n("checking") + "..." : n("lblProceed")
                }), (0,
                t.jsx)(r.motion.button, {
                    onClick: M,
                    disabled: !k,
                    className: `text-white mb-2 mt-2 md:ml-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition ${k ? "bg-[#28a745] hover:bg-[#218838] focus:ring-[#218838] cursor-pointer" : "bg-gray-400 cursor-not-allowed"}`,
                    children: k ? n("resendOtp") : n("resendOtp") + ` ${w}s`
                })]
            })]
        })
    }
      , J = () => {
        let e = (0,
        s.useSelector)(e => e.authenticationStep.step);
        return (0,
        t.jsxs)(t.Fragment, {
            children: [1 === e && (0,
            t.jsx)(x, {}), 2 === e && (0,
            t.jsx)(O, {}), 3 === e && (0,
            t.jsx)(M, {}), 4 === e && (0,
            t.jsx)(H, {}), 5 === e && (0,
            t.jsx)(G, {}), 6 === e && (0,
            t.jsx)(S, {}), 7 === e && (0,
            t.jsx)($, {}), 8 === e && (0,
            t.jsx)(V, {}), 9 === e && (0,
            t.jsx)(L, {}), 10 === e && (0,
            t.jsx)(F, {}), 100 === e && (0,
            t.jsx)(P, {})]
        })
    }
      , q = ({onClose: e}) => {
        let[s,l] = (0,
        a.useState)(!1);
        return (0,
        a.useEffect)( () => {
            let e = setTimeout( () => {
                l(!0)
            }
            , 1e3);
            return () => clearTimeout(e)
        }
        , []),
        (0,
        t.jsx)(t.Fragment, {
            children: (0,
            t.jsx)(r.motion.div, {
                initial: {
                    opacity: 0,
                    scale: .95
                },
                animate: {
                    opacity: 1,
                    scale: 1
                },
                exit: {
                    opacity: 0,
                    scale: .95
                },
                transition: {
                    duration: .3
                },
                className: "fixed inset-0 backdrop-blur-sm backdrop-brightness-75 flex justify-center items-center z-50",
                children: (0,
                t.jsx)("div", {
                    className: "fixed inset-0 backdrop-blur-sm backdrop-brightness-75 flex justify-center items-center z-50",
                    children: (0,
                    t.jsxs)("div", {
                        className: "bg-white max-h-[90vh] w-full max-w-2xl p-6 rounded-lg overflow-y-auto scrollbar-hide relative",
                        children: [s && (0,
                        t.jsx)("button", {
                            onClick: e,
                            className: "absolute top-2 right-2 text-gray-500 hover:text-red-500 hover:cursor-pointer mb-4",
                            children: "✕"
                        }), (0,
                        t.jsxs)("div", {
                            className: "space-y-4 text-sm text-gray-800",
                            children: [(0,
                            t.jsx)("div", {
                                className: "w-full text-red-500 text-center font-bold text-lg bg-white z-100",
                                children: "জরুরি বিজ্ঞপ্তি"
                            }), (0,
                            t.jsxs)("div", {
                                className: "text-gray-700 font-light text-xs",
                                children: [(0,
                                t.jsxs)("p", {
                                    children: ["ভিসা পেমেন্ট এর পূর্বে ভিডিও টি দেখে নিন।", " ", (0,
                                    t.jsx)("a", {
                                        href: "https://www.youtube.com/watch?v=ckFsAX727LM&source_ve_path=MTc4NDI0",
                                        target: "_blank",
                                        rel: "noopener noreferrer",
                                        className: "text-blue-600 hover:text-blue-700 hover:underline",
                                        children: "(ভিডিও লিংক)"
                                    })]
                                }), (0,
                                t.jsxs)("p", {
                                    children: ["Please watch the video before proceeding for visa payment.", (0,
                                    t.jsx)("a", {
                                        href: "https://www.youtube.com/watch?v=ckFsAX727LM&source_ve_path=MTc4NDI0",
                                        target: "_blank",
                                        rel: "noopener noreferrer",
                                        className: "text-blue-600 hover:text-blue-700 hover:underline",
                                        children: "(Video Link)"
                                    })]
                                })]
                            }), (0,
                            t.jsx)("div", {
                                className: "text-black font-semibold text-sm",
                                children: "ভিসা আবেদন পত্র জমা দেয়ার স্লট নেয়ার জন্য কি করণীয় এবং কি করবেন না"
                            }), (0,
                            t.jsxs)("div", {
                                className: "text-black font-normal text-sm my-1",
                                children: [(0,
                                t.jsx)("p", {
                                    children: "১. বুকিংয়ের জন্য স্লট খোলার আগে সমস্ত নথি প্রস্তুত রাখুন।"
                                }), (0,
                                t.jsx)("p", {
                                    children: "[অনলাইন আবেদনপত্র, ব্যক্তিগত যোগাযোগ নম্বর সহ মোবাইল হ্যান্ডসেট (OTP যাচাইকরণের জন্য), আবেদনকারীর ছবির ফরমেটে হবে jpg/png(সাইজ হবে সর্বোচ্চ ২ এমবি)]এগুলো ডেস্কটপ/মোবাইলে সংরক্ষিত রাখুন যেন রেজিস্ট্রেশনের সময় সহজে আপলোড করতে পারেন ।"
                                }), (0,
                                t.jsx)("p", {
                                    children: "২. প্রথমবার নিবন্ধনের জন্য লগইন আইডি এবং পাসওয়ার্ড তৈরি করতে হবে।"
                                }), (0,
                                t.jsx)("p", {
                                    children: "৩. একটি একক বুকিং সেশনের অধীনে একটি একক লগইন আইডির মাধ্যমে সর্বাধিক ৫ জনের বুকিং করা যেতে পারে।"
                                }), (0,
                                t.jsx)("p", {
                                    children: "৪. একটি একক লগইন আইডির মাধ্যমে বুক করা স্লটগুলি একই ভিসা আবেদনকারি হতে হবে৷ একই লগইন আইডির মাধ্যমে অসংলগ্ন অ্যাপ্লিকেশন লক্ষ্য করা গেলে পেমেন্ট ইনভয়েস বাতিল করা হবে।"
                                }), (0,
                                t.jsx)("p", {
                                    children: "৫. স্লট বুকিং নিয়ে এগিয়ে যাওয়ার আগে অ্যাপ্লিকেশন তথ্য ট্যাবে পরিবারের সকল সদস্যদের তথ্য যোগ করুন ।"
                                }), (0,
                                t.jsx)("p", {
                                    children: "৬. অনুগ্রহ করে ১৫ মিনিটের মধ্যে আপনার স্লট বুকিং সম্পূর্ণ করুন,না করতে পরলে সেশনের মেয়াদ শেষ হবে এবং নতুন লগইন প্রয়োজন হবে।"
                                }), (0,
                                t.jsx)("p", {
                                    children: "৭. আবেদনপত্র IVAC-সেন্টারে জমা দেওয়ার সময় আপনার ব্যক্তিগত যোগাযোগ নম্বর সঙ্গে রাখুন।যা OTP যাচাইকরণ এবং অ্যাপয়েন্টমেন্ট বুকিংয়ের জন্য ব্যবহৃত হয়েছে।"
                                }), (0,
                                t.jsx)("p", {
                                    children: "৮. ভারতীয় ভিসার জন্য কোন অনুমোদিত এজেন্ট বা দালাল নেই।এটি একই সাথে সংযুক্তি আত্মসাৎ এবং চাঁদাবাজির ঝুঁকি বহন করে।"
                                }), (0,
                                t.jsx)("p", {
                                    children: "৯. এটি একটি গুরুত্বপূর্ণ বিষয়, যেখানে অসাধু উপাদানগুলি একাধিক পেমেন্ট চ্যানেল ব্যবহার করে কৃত্রিম ট্র্যাফিক তৈরি করছে এবং প্রকৃত আবেদনকারীদের জন্য সমস্যা সৃষ্টি করছে। এই ধরনের কার্যকলাপের বিরুদ্ধে কঠোর ব্যবস্থা গ্রহণ করা উচিত এবং যথাযথ পদক্ষেপ গ্রহণ করা দরকার, যেমন একাধিক পেমেন্টের ক্ষেত্রে ফেরত না দেওয়া। এটি কেবল অসাধু কার্যকলাপের প্রতি একটি কঠিন বার্তা পাঠাবে না, বরং প্রকৃত আবেদনকারীদের জন্য সিস্টেমটিকে আরও সুষ্ঠু এবং কার্যকরী করতে সাহায্য করবে।"
                                })]
                            }), (0,
                            t.jsx)("div", {
                                className: "w-2/3 h-0.5 bg-red-600 mx-auto"
                            }), (0,
                            t.jsx)("div", {
                                className: "text-black font-semibold text-sm",
                                children: "Do’s and Don’ts (Online Payment and Appointment)"
                            }), (0,
                            t.jsxs)("div", {
                                className: "text-black font-normal text-sm py-1",
                                children: [(0,
                                t.jsx)("p", {
                                    children: "1. Keep all documents ready, before slot opens for booking. [Online application form, mobile handset with personal contact no.(for OTP verification), applicant photograph in jpg, png format (max 2MB size) saved in Desktop/mobile for easy upload during registration.]"
                                }), (0,
                                t.jsx)("p", {
                                    children: "2. Login ID and password will have to be created for first time registration before proceeding with appointment."
                                }), (0,
                                t.jsx)("p", {
                                    children: "3. Maximum 5 persons’ booking can be done through one single login ID under a single booking session."
                                }), (0,
                                t.jsx)("p", {
                                    children: "4. The booked slots through one single Login ID should be for the same Visa application/journey. Unrelated applications through same login ID, under 1 payment invoice will be cancelled."
                                }), (0,
                                t.jsx)("p", {
                                    children: "5. Add all family members in Application Info tab before proceeding with slot booking."
                                }), (0,
                                t.jsx)("p", {
                                    children: "6. Please complete total slot booking within 15 minutes, after which session will expire and fresh login will be required."
                                }), (0,
                                t.jsx)("p", {
                                    children: "7. During Submission at IVAC on Appointment date, invariably carry your personal contact no. used for OTP validation and appointment booking."
                                }), (0,
                                t.jsxs)("p", {
                                    children: ["8.", " ", (0,
                                    t.jsx)("span", {
                                        className: "font-bold",
                                        children: "There are no authorized agents or brokers for Indian Visa."
                                    }), " ", "Association with the same carries risk of embezzlement and extortion."]
                                }), (0,
                                t.jsx)("p", {
                                    children: "9. It has been reported that some unscrupulous elements are charging large sums of money for deploying several personnel at the same time to attempt payments through multiple channels creating unnecessary traffic and causing problems to genuine applicants. In such cases, multiple payments made will not be refunded as a step to discourage such malpractices."
                                })]
                            })]
                        })]
                    })
                })
            })
        })
    }
    ;
    function X({lblTabMenuAuth: e, steps: s}) {
        let[r,l] = (0,
        a.useState)(!1);
        return (0,
        a.useEffect)( () => {
            sessionStorage.getItem("noticeShown") || l(!0)
        }
        , []),
        (0,
        t.jsxs)(t.Fragment, {
            children: [r && (0,
            t.jsx)(q, {
                onClose: () => {
                    l(!1),
                    sessionStorage.setItem("noticeShown", "true")
                }
            }), (0,
            t.jsxs)("div", {
                className: "flex flex-col bg-[#b5d6a9] sm:mx-8 lg:mx-20 rounded-md shadow-lg border-[0.1px] border-slate-300 overflow-auto",
                children: [(0,
                t.jsxs)("div", {
                    className: "flex flex-row gap-4 text-xs overflow-x-scroll border-b-[0.1px] border-slate-300 scroll-smooth scrollbar-hide",
                    children: [(0,
                    t.jsx)("p", {
                        className: "font-bold bg-white rounded-tl py-4 px-4",
                        children: e
                    }), s.map(e => (0,
                    t.jsxs)("p", {
                        className: "py-2 px-4 flex items-center text-[#5c7b2f] font-bold",
                        children: [(0,
                        t.jsx)("span", {
                            className: "bg-white rounded-full px-2 py-1 mr-2 text-xs font-bold",
                            children: (0,
                            t.jsx)("i", {
                                children: e.id
                            })
                        }), e.name]
                    }, e.id))]
                }), (0,
                t.jsx)("div", {
                    className: "bg-white w-full py-1 md:px-2 lg:px-4",
                    children: (0,
                    t.jsx)(J, {})
                })]
            })]
        })
    }
    e.s(["default", () => X], 48949)
}
]);
